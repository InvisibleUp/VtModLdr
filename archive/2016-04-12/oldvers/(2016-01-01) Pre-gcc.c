 ...
 * =====================================================================================
 */
char * SelectFile(HWND hwnd, int *nameOffset, char *Filter)
{
	OPENFILENAME ofn = {0};
 ...
 * =====================================================================================
 */
int GetUsedSpaceBytes(char *ModUUID, int File)
{
	int add, clear;
	sqlite3_stmt *command = NULL;
...
int SQL_HandleErrors(int SQLResult)
{
	if(SQLResult != SQLITE_OK){
		//AlertMsg("Internal Database Error!", sqlite3_errstr(SQLResult));
		AlertMsg("Internal Database Error!", "");
	}
	return SQLResult;
}
 ...
 * =====================================================================================
 */
signed long JSON_GetInt(json_t *root, char *name)
{
 ...
 * =====================================================================================
 */
BOOL File_Known(char *FileName, json_t *whitelist)
{
 ...
 * =====================================================================================
 */
int File_OpenSafe(char *filename, int flags)
{
	int handle;
	int accessflag;
 ...
 * =====================================================================================
 */
void File_Copy(char *OldPath, char *NewPath)
{
...
{	
	...
	if(!deps || !json_is_array(deps)){return TRUE;}
	...
	json_array_foreach(deps, i, value){
		...
		if (ModCount == 0){
			...
			sqlite3_stmt *command;
			const char *query = "DELETE FROM Dependencies "
			                          "WHERE ParentUUID = ?;";
			if(SQL_HandleErrors(
				sqlite3_prepare_v2(CURRDB, query, -1, &command, NULL)
			   ) != 0){return FALSE;}
...
void Mod_CheckDepAlert(json_t *root){
	...
	deps = json_object_get(root, "dependencies");
	if (!deps == NULL || !json_is_array(deps)){
		free(message); 
		return;
	}
...
int File_GetID(char * FileName)
{
	...
	if(ID == -1){
		if(CURRERROR != errNOERR){return -1;} //-1 because error. Bail.
...
int File_MakeEntry(char *FileName){
	...
	sqlite3_finalize(command);
	//Check that GetNum succeeded
	if(IDCount == -1){CURRERROR = errCRIT_DBASE;}
...
struct VarValue Var_GetValue_SQL(char *VarUUID){
	...
	if(SQL_HandleErrors(
		sqlite3_prepare_v2(CURRDB, query, -1, &command, NULL)
	   ) != 0){result.type = INVALID; return result;}
	if(SQL_HandleErrors(
...
struct ModSpace Mod_FindSpace(struct ModSpace input, int len, char *ModUUID, BOOL Clear){
	...
	//Pull the data
	if(JSON_GetStrLen(row, "ID") < 1){ //No ID
		IDStr = strdup("");
	} else {
...
BOOL Mod_FindUUIDLoc(int *start, int *end, char *UUID)
{
	sqlite3_stmt *command;
	json_t *out, *row;
...
BOOL Mod_Install(json_t *root)
{
	...
	json_array_foreach(patchArray, i, patchCurr){
		...
		if(
			...
			//CURRERROR = errWNG_MODCFG; //Don't set. It's been handled
			ProgDialog_Kill(ProgDialog);
			return FALSE;
		}
		...
		if(strcmp(Mode, "Repl") == 0){
			...
			if (ModOp_Add(input, file, bytes, len, ModUUID, FALSE) == FALSE){
				close(file);
				retval = FALSE;
				ErrCracker(CURRERROR);
				break;
			...
			if (ModOp_Add(input, file, bytes, len, ModUUID, TRUE) == FALSE){
				close(file);
				retval = FALSE;
				ErrCracker(CURRERROR);
				break;
...
BOOL Mod_Uninstall(char *ModUUID)
{
	...
	if(strcmp(ModUUID, "") != 0){
		...
		AlertMsg("The mod database appears to be corrupted.\n"
			 "We'll try our best to keep everything running, but\n"
...
void SQL_Load(){
	...
	assert(result == SQLITE_OK); //Replace with something better
