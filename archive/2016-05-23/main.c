...
BOOL Var_MakeEntry(struct VarValue result, const char *ModUUID)
{
	CURRERROR = errNOERR;
...
BOOL Var_MakeEntry_JSON(json_t *VarObj, const char *ModUUID)
{
	...
	//Insert via SQL
	retval = Var_MakeEntry(result, ModUUID);
...
BOOL CALLBACK Dlg_Var_Save(HWND hCtl, LPARAM lParam)
{
	...
	if (strcmp(CtlType, "Numeric") == 0 || strcmp(CtlType, "HexNum") == 0){
		char NumText[128];
...
LRESULT CALLBACK Dlg_Var(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CLOSE:
		...
		case IDOK:{
			...
			EnumChildWindows(hwnd, Dlg_Var_Save, NULL);
			DestroyWindow(hwnd);
		break;
		}
	...
	json_array_foreach(VarArray, i, VarCurr_JSON){
		...
		if(
			...
			if(strcmp(VarCurr.publicType, "HexNum") == 0){
				...
				case Int32:
				case uInt32:
					SendMessage(hSpinControl, UDM_SETRANGE32, 0, LONG_MAX);
				...
				case Int8:
					...
					break;
				case uInt32:
					SendMessage(hSpinControl, UDM_SETRANGE32, 0, LONG_MAX); //Only goes up to 0x7FFFFFF. Ugh.
					SendMessage(hSpinControl, UDM_SETPOS32, 0, VarCurr.uInt32);
					break;
				case uInt16:
					SendMessage(hSpinControl, UDM_SETRANGE32, 0, USHRT_MAX);
