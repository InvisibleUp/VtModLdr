...
BOOL SQL_Load(){
	...
	if(SQL_HandleErrors(__FILE__, __LINE__, 
		...
		sqlite3_exec(CURRDB, 
			...
			"CREATE TABLE IF NOT EXISTS `VarWritePos` ("
			"`Var`          	TEXT NOT NULL,"
			"`Start`         	INTEGER NOT NULL,"
...
BOOL SQL_Populate(json_t *GameCfg)
{
	...
	// Create "mods" folder (Windows 7 needs this. 2000 is fine, weirdly enough.)
	mkdir(ModDir); // Might throw an error, but it's platform-specific and likely "file exists"
	safe_free(ModDir);
	...
	json_array_foreach(out, i, row){
		...
		NewSpc.End = JSON_GetuInt(row, "End") - 1;
...
int WINAPI WinMain (HINSTANCE hInst, HINSTANCE hPrevInst, char * cmdParam, int cmdShow)
{
	...
	if(!CONFIG.CURRPROF){
		Interface_EditConfig();
		if(
			CURRERROR == errUSR_ABORT ||
			strndef(CONFIG.CURRDIR)
		){
			// User wants to quit
			return 0;
		}
	...
	if(CURRERROR == errNOERR){
		char *FilePath = NULL;
		char *GameName = NULL;
		GameName = File_GetName(1);
		asprintf(&FilePath, "%s/%s", CONFIG.CURRDIR, GameName);

		CONFIG.CHECKSUM = crc32File(GameName);
