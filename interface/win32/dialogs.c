...
BOOL CALLBACK Dlg_Var_Save(HWND hCtl, LPARAM lParam)
{
	...
	// Reinstall needed patches
	Var_RePatch(newVar.UUID);
 ...
 *                WMX_ERROR:      Handles any errors that may occur.
 *
...
{	
	switch(Message){
	case WM_INITDIALOG:{
		struct ProgConfig *LocalConfig = Profile_Clone();
		if (!LocalConfig) {
			SendMessage(hwnd, WMX_ERROR, 0, 0);
		}
	...
	case WM_COMMAND:
		...
		case IDOK:{
			...
			if (
				!LocalConfig || !LocalConfig->CURRPROF ||
		...
		case IDC_CFGGAMEBROWSE:{
			...
			//Get directory (needed for GameVer)
			...
			//Get game version
			GameVer = Profile_GetGameVerID(cfgpath, LocalConfig);
	...
	case WMX_RELOADPROF:{
		...
		GameVer = Profile_GetGameVer(LocalConfig->GAMECONFIG, LocalConfig);
...
BOOL Dlg_Main_UpdateDetails(HWND hwnd, int listID)
{
	...
	if(noResult == FALSE){
		...
		name = JSON_GetStr(row, "Name");
		auth = JSON_GetStr(row, "Author");
		cat = JSON_GetStr(row, "Category");
		desc = JSON_GetStr(row, "Info");
		ver = JSON_GetStr(row, "Version");
...
/*
* ===  FUNCTION  ======================================================================
*         Name:  Dlg_Main_Repaint
*  Description:  Repaints the main dialog, including the preview image and 
*                colored background if applicable.
* =====================================================================================
*/
void Dlg_Main_Repaint(HWND hwnd)
{
	PAINTSTRUCT ps;
	HDC hdc;
	BITMAP bitmap;
...
BOOL Mod_InstallPrep(HWND hwnd)
{
	...
	if(extract(fpath, temppath) == FALSE){
		AlertMsg("There was an error extracting the ZIP file", "Mod Config Error");
 ...
 *
 *                Future versions may end up in the variable window being rolled into the
